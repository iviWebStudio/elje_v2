import {QUERY_STATE_STORE_KEY as storeKey} from '@elje/block-data';
import {
	useDispatch,
	useSelect,
} from '@wordpress/data';
...
/**
 * A custom hook that works similarly to useQueryStateByContext.
 * However, this hook allows for synchronizing with a provided queryState object.
 *
 * This hook does the following things with the provided `synchronizedQuery` object:
 *
 * - whenever synchronizedQuery varies between renders, the queryState will be
 *   updated to a merged object of the internal queryState and the provided object.
 *   Any values from the same properties between objects will be set from synchronizedQuery.
 * - if there are no changes between renders, then the existing internal queryState is always returned.
 ...
 * affect query state).
 *
 * @param {Object} synchronizedQuery A provided query state object to synchronize internal query state with.
 * @param {string} [context]         What context to retrieve the query state for. If not provided,
 * will be pulled from the QueryStateContextProvider in the tree.
 */
