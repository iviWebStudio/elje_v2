 ...
 * to `rc`.
 *
 * @param {string} setting Setting name (e.g. wpVersion or eljeVersion).
...
/**
 * Compare the current ELJE version with the provided `version` param using the `operator`.
 *
 */
export const isEljeVersion = (
	version: string,
	operator: compareVersions.CompareOperator = '='
): boolean => {
	return compareVersionSettingIgnorePrerelease(
